	...
	xhttp2.onreadystatechange = function() {//funzione che gestisce la lo stato della risposta e la risposta
		if (this.readyState == 4 && this.status == 200) {//controllo che lo stato della risposta sia 4 e che sia stata elaborata con successo
			...
			switch(this.responseText){
				...
				case '3': messaggio = "Hai raggiunto il numero massimo di questo articolo per ordinazione"; break;
			}
			alert(messaggio);
...
function aggiornaCarrello(codice,agg){
	var xhttp3 = new XMLHttpRequest();//istanzio l'oggetto che permette di gestire le richieste ajax

	xhttp3.onreadystatechange = function() {//funzione che gestisce la lo stato della risposta e la risposta
		if (this.readyState == 4 && this.status == 200) {//controllo che lo stato della risposta sia 4 e che sia stata elaborata con successo
			var messaggio;
			switch(this.responseText){
				case '1': messaggio = "Errore nell'aggiornamento dell'articolo al carrello"; break;
				case '2': messaggio = "Devi accedere per aggiungere articoli al carrello"; break;
				case '3': messaggio = "Hai raggiunto il numero massimo di questo articolo per ordinazione"; break;
			}
			if(this.responseText != '0') alert(messaggio);
			if(this.responseText == '2'){
				location.replace("login.php");
			}else if(this.responseText == '0'){
				document.getElementById("totaleCarrello").innerHTML = Number(document.getElementById("totaleCarrello").innerHTML) + (Number(document.getElementById("P"+codice).innerHTML)*Number(agg));
				document.getElementById("Q"+codice).innerHTML = Number(document.getElementById("Q"+codice).innerHTML) + Number(agg);
				if(document.getElementById("Q"+codice).innerHTML=='0') location.replace("carrello.php");
			}
		}
	};
  	xhttp3.open("POST", "aggiornaCarrello.php", true);//imposto il tipo di richiesta(POST o GET e sincrona(false) o asincrona(true)) e la pagina da interrogare
  	xhttp3.setRequestHeader("Content-type", "application/x-www-form-urlencoded");//imposto l'header della richiesta in POST
  	xhttp3.send("codice="+codice+"&agg="+agg);//invio la richiesta passando due parametri. 'nomeDato' indica di che campo si vuole fare il controllo('Username' o 'Email') e 'valDato' è il valore scelto dall'utente per quel campo.
}